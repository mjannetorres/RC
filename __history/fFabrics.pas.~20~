...
begin
  ...
  begin
    ...
    begin
      qry_Fabric.Close;
      qry_Fabric.SQL[2] := 'WHERE ID = :ID';
      qry_Fabric.ParamByName('ID').Value  := brw_FabricsID.Value;
      qry_Fabric.Open();

      qry_Fabric.Edit;
      qry_FabricCANCELLED.Value       := True;
      qry_FabricDELETEDBYID.Value     := userid;
      qry_FabricDELETEDDATETIME.Value := Now;
      qry_Fabric.Post;
...
begin
  ...
  begin
    ...
    f_FabricTemp  := Tf_FabricTemp.Create(Self);
    f_FabricTemp.Caption  := qry_FabricDESCRIPTION.AsString;
...
begin
  ...
  begin
    ...
    f_FabricTemp  := Tf_FabricTemp.Create(Self);
    f_FabricTemp.Caption  := 'New Fabric';
    if f_FabricTemp.ShowModal =  mrOk then
    begin
      qry_Fabric.Post;
      qry_Fabric.ApplyUpdates();
