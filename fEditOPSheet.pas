...
procedure Tf_EditOPSheet.DeleteExecute(Sender: TObject);
begin
  with dm_PM do
  begin
     ...
     begin
       ...
       qry_Logs.Close;
       qry_Logs.SQL[2]  := 'WHERE EMPID = :EMPID AND  JONO = :JONO AND SHIRTID = :ID AND SPRINT = :SPRINT AND SIZE = :SIZE AND FIT = :FIT AND CANCELLED = FALSE';
       qry_Logs.ParamByName('JONO').Value    := qry_PBJONO.Value;
       qry_Logs.ParamByName('ID').Value      := tb_OPDetailSHIRTID.Value;
       qry_Logs.ParamByName('SPRINT').Value  := qry_PBSPRINT.Value;
       qry_Logs.ParamByName('SIZE').Value    := tb_OPDetailSIZE.Value;
       qry_Logs.ParamByName('FIT').Value     := tb_OPDetailFIT.Value;
       qry_Logs.ParamByName('EMPID').Value   := tb_OPDetailEMPID.Value;
       qry_Logs.Open();

       if qry_Logs.RecordCount > 0 then
       begin
         MessageDlg('The selected record has been logged by worker assigned. Cannot delete this record', mtError, [mbOK], 0);
         Exit
       end;
