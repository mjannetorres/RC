...
procedure Tf_ExpenseTemp.chk_postedClick(Sender: TObject);
begin
  if chk_posted.Checked then
  begin
    if manageui then
    dm_PM.qry_ExpensePOSTEDDATETIME.Value  := Now
    else
    begin
      dm_PM.qry_ExpensePOSTEDDATETIME.Clear;
      dm_PM.qry_ExpensePOSTED.Value  := False;
      chk_posted.Checked        := False;
    end;

    if dm_PM.tb_ExpenseDetail.RecordCount = 0 then
    MessageDlg('Details cannot be empty!', mtError, [mbOK], 0)
    else if (dm_PM.qry_ExpenseNETAMOUNT.Value = 0) or (dm_PM.qry_ExpenseNETAMOUNT.IsNull) then
    MessageDlg('Net amount cannot be 0!', mtError, [mbOK], 0)
    else if dm_PM.qry_ExpenseEXPENSEDATE.IsNull then
    MessageDlg('Date cannot be empty!', mtError, [mbOK], 0)
    else if (dm_PM.qry_ExpenseNOTES.IsNull) or (Trim(dm_PM.qry_ExpenseNOTES.AsString) = '') then
    MessageDlg('Notes cannot be empty!', mtError, [mbOK], 0)
    else if (dm_PM.qry_ExpensePAYEE.IsNull) or (Trim(dm_PM.qry_ExpensePAYEE.AsString) = '') then
    MessageDlg('Payee cannot be empty!', mtError, [mbOK], 0)
    else if dm_PM.qry_ExpensePAYMENTMODE.Value = 'CHECK' then
    begin
      if dm_PM.qry_ExpenseBANKID.IsNull then
      MessageDlg('Bank cannot be empty!', mtError, [mbOK], 0)
      else if dm_PM.qry_ExpenseCHECKNO.IsNull then
      MessageDlg('Check No. cannot be empty!', mtError, [mbOK], 0)
      else if dm_PM.qry_ExpenseCHECKDATE.IsNull then
...
begin
  ...
  begin
    ...
    brw_Emp.ParamByName('ID').Value := qry_ExpenseCREATEDBYID.Value;
    brw_Emp.Open();

    f_reports := Tf_Reports.Create(Self);
    TfrxMemoView(f_Reports.rep_Expense.FindObject('bank')).Visible      := qry_ExpensePAYMENTMODE.Value = 'CHECK';
    TfrxMemoView(f_Reports.rep_Expense.FindObject('lblbank')).Visible   := qry_ExpensePAYMENTMODE.Value = 'CHECK';
    TfrxMemoView(f_Reports.rep_Expense.FindObject('ptbank')).Visible    := qry_ExpensePAYMENTMODE.Value = 'CHECK';
    TfrxMemoView(f_Reports.rep_Expense.FindObject('checkno')).Visible   := qry_ExpensePAYMENTMODE.Value = 'CHECK';
    TfrxMemoView(f_Reports.rep_Expense.FindObject('lblcheckno')).Visible:= qry_ExpensePAYMENTMODE.Value = 'CHECK';
    TfrxMemoView(f_Reports.rep_Expense.FindObject('ptcheckno')).Visible := qry_ExpensePAYMENTMODE.Value = 'CHECK';
    TfrxMemoView(f_Reports.rep_Expense.FindObject('check')).Visible     := qry_ExpensePAYMENTMODE.Value = 'CHECK';
    TfrxMemoView(f_Reports.rep_Expense.FindObject('ptcheck')).Visible   := qry_ExpensePAYMENTMODE.Value = 'CHECK';
    TfrxMemoView(f_Reports.rep_Expense.FindObject('lblcheck')).Visible  := qry_ExpensePAYMENTMODE.Value = 'CHECK';
    f_Reports.rep_Expense.ShowReport(True);
  end;
end;
...
begin
  with dm_PM do
  begin
    if (qry_Expense.State = dsEdit) or (qry_Expense.State = dsInsert) then
    begin
