...
begin
  ...
  begin
    ...
    begin
      ...
      qry_Rights.Close;
      qry_Rights.SQL[2] := 'WHERE POLICYID = :POLICY AND ROLEID = :ID';
      qry_Rights.ParamByName('ID').Value      := brw_rolesID.Value;
      qry_Rights.ParamByName('POLICY').Value  := brw_PolicyID.Value;
      qry_Rights.Open();
...
begin
  with dm_PM do
  begin
    if brw_Policy.Active then
    id  := brw_PolicyID.Value
    else
    id  := 0;
