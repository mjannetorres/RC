...
begin
  ...
  begin
    ...
    begin
      if MessageDlg('Continue to delete PO No. '+brw_POID.AsString +'?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
      begin
        qry_PO.Close;
...
begin
  ...
  begin
    ...
    f_POTemp := Tf_POTemp.Create(Self);
    f_POTemp.PrintExecute(nil);
  end;
end;

procedure Tf_PO.sDBTableView1CellClick(Sender: TcxCustomGridTableView;
  ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton;
  AShift: TShiftState; var AHandled: Boolean);
begin
  btnPrint.Visible  := dm_PM.brw_POPOSTED.Value;
end;
