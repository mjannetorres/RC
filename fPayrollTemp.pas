...
begin
  ...
  begin
    ...
    f_PayrollComp.Caption := 'Period Covered';
    if f_PayrollComp.ShowModal = mrOk then
    begin
      date1 := f_PayrollComp.cxDateEdit1.Date;
      date2 := f_PayrollComp.cxDateEdit2.Date;
      ...
      begin
        tb_PaySlip.Append;
        tb_PaySlipIDNO.Value        := tb_PayDetailIDNO.Value;
        tb_PaySlipRATE.Value        := tb_PayDetailRATE.Value;
        tb_PaySlipALLOWANCES.Value  := tb_PayDetailALLOWANCES.Value;
        tb_PaySlipBACKWAGE.Value    := tb_PayDetailBACKWAGE.Value;
        tb_PaySlipOTHERS.Value      := tb_PayDetailOTHERS.Value;
        tb_PaySlipSSS.Value         := tb_PayDetailSSS.Value;
        tb_PaySlipPAGIBIG.Value     := tb_PayDetailPAGIBIG.Value;
        tb_PaySlipPHIC.Value        := tb_PayDetailPHIC.Value;
        tb_PaySlipNODAYS.Value      := tb_PayDetailNODAYS.Value;
        tb_PaySlipNOPIECES.Value    := tb_PayDetailNOPIECES.Value;
        tb_PaySlipEMPNAME.Value     := tb_PayDetailEMPNAME.Value;
        tb_PaySlipCA.Value          := tb_PayDetailCA.Value;
        tb_PaySlipMODE.Value        := tb_PayDetailMODE.Value;
        tb_PaySlipHRSLATE.Value     := tb_PayDetailHRSLATE.Value;
        tb_PaySlipDAYSABSENT.Value  := tb_PayDetailDAYSABSENT.Value;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              tb_PaySlipJODATE.Value      := brw_LogsLOGDATE.Value;
              tb_PaySlipJONO.Value        := brw_LogsJONO.Value;
              tb_PaySlipITEM.Value        := brw_LogsDESCRIPTION.Value;
              tb_PaySlipQTY.Value         := brw_LogsQTY.Value;
              ...
              if brw_LogsROLEID.Value = 1 then
              tb_PaySlipPRICE.Value := (brw_LogsQTY.AsFloat * brw_GarmentDescARTISTSHARE.Value)
              else if brw_LogsROLEID.Value = 2 then
              tb_PaySlipPRICE.Value := (brw_LogsQTY.AsFloat * brw_GarmentDescPRINTOPSHARE.Value)
              else if brw_LogsROLEID.Value = 3 then
              tb_PaySlipPRICE.Value := (brw_LogsQTY.AsFloat * brw_GarmentDescCUTTERSHARE.Value)
              else if brw_LogsROLEID.Value = 4 then
              tb_PaySlipPRICE.Value :=  (brw_LogsQTY.AsFloat * brw_GarmentDescHEATPRESSSHARE.Value)
              else if brw_LogsROLEID.Value = 5 then
              tb_PaySlipPRICE.Value :=  (brw_LogsQTY.AsFloat * brw_GarmentDescTAILORSHARE.Value);
